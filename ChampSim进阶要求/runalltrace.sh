...
for trace_file in $traces; do
	...
	# 提取IPC
	ipc=$(grep -m 1 "CPU 0 cumulative IPC" "$temp_output" | awk '{print $5}')

	# 确保我们有一个非空的IPC值
	if [ ! -z "$ipc" ]; then
		echo "IPC for $trace: $ipc"
	else
		echo "IPC for $trace: Data not found"
	fi
